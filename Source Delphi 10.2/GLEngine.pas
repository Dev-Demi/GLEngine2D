...
//  CVec = CVert;
  ...
  TVBOPrimitive = class
     ...
     public
      ...
      function IsExtensionSupported(szTargetExtension: AnsiString): boolean;
    end;
...
//   gmf: array [0..255] of GLYPHMETRICSFLOAT; // ìàññèâ òðåáóåòñÿ äëÿ âåêòîðíîãî
 ...
 //  FontHandle: Integer;
  ...
  {+}Procedure TextOut(x,y:single; text:AnsiString; angle:single=0);
     Procedure TextOutUseImageFont(x,y:single; text:string; Font:Cardinal; angle:single=0;wchar:single=64;hchar:single=64);
  {+}Procedure KillFont;
  ...
  {+}Procedure  LoadImage(Filename: String; var Texture : Cardinal; LoadFromRes : Boolean);
  {+}Procedure DrawImage(x,y,w,h,Angle:single;Center,tile:boolean;Image:Cardinal);
  {+}Procedure DrawImageArea(x,y,w,h,Tx1,Ty1,Tx2,Ty2:single;Image:Cardinal);
...
procedure png8to24(png: TPNGImage);
var
 tmp: TPNGImage;
 tRNS: TChunktRNS;
 PLTE: TChunkPLTE;
...
procedure TGLEngine.DrawImageArea(x, y, w, h, Tx1, Ty1, Tx2, Ty2:single;
  Image: Cardinal);
begin
 glMatrixMode(GL_TEXTURE);
  glLoadIdentity();
 ...
 glPushMatrix();
  glTranslated(x,y,0);
  glBegin(GL_QUADS);
   glTexCoord2f(tx1,ty1);  glVertex3f(0, 0, 0);
   glTexCoord2f(tx2,ty1);  glVertex3f(w, 0, 0);
   glTexCoord2f(tx2,ty2);  glVertex3f(w, h, 0);
   glTexCoord2f(tx1,ty2);  glVertex3f(0, h, 0);
  glEnd;
 glPopMatrix();
 glDisable(GL_TEXTURE_2D);
end;
...
Procedure TGLEngine.LoadImage(Filename: String; var Texture: Cardinal;
  LoadFromRes: Boolean);
begin
...
////////////////////////
  ...
  glCallLists(length(text),GL_UNSIGNED_BYTE,PAnsiChar(text)); // Vykreslí display listy
  glPopMatrix();
  glPopAttrib;
end;

procedure TGLEngine.TextOutUseImageFont(x, y: single; text: string;
...
function TVBOPrimitive.IsExtensionSupported(
  szTargetExtension: AnsiString): boolean;
var
  pszExtensions: AnsiString;
  pszWhere: integer;
begin
  pszWhere := Pos(AnsiString(' '),szTargetExtension);

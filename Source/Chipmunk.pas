...
Procedure SetGlEngine(GLEng:TGLEngine);
...
// ADDITIONAL FUNCTIONS FOR GLEngine
  ...
  case shape.klass._type of
    CP_CIRCLE_SHAPE:
      with PcpCircleShape( shape )^ do
        begin
          GLE.PointSize(r*2);
        //  GLE.SetColor(TGLColor(shape.data^));
          GLE.Point(tc.x, tc.y);
      //    GLE.Ellipse(tc.x, tc.y, r,r,0.1,0,32);
      //    GLE.Line(tc.x, tc.y, tc.x + shape.body.rot.x * r, tc.y + shape.body.rot.y * r);
      ...
      with PcpPolyShape( shape )^ do
        begin
          for i := 0 to numVerts - 2 do
           GLE.Line(tverts[ i ].x, tverts[ i ].y, tverts[ i + 1 ].x, tverts[ i + 1 ].y) ;
       ...
       //   pr2d_Line( tverts[ numVerts - 1 ].x, tverts[ numVerts - 1 ].y, tverts[ 0 ].x, tverts[ 0 ].y, PLongWord( data )^, 255, PR2D_SMOOTH );
           GLE.Line(tverts[ numVerts - 1 ].x, tverts[ numVerts - 1 ].y, tverts[ 0 ].x, tverts[ 0 ].y) ;
        end;
  end;
end;
...
//      pr2d_Circle( v.x, v.y, 4, PLongWord( data )^, 255, 8, PR2D_SMOOTH or PR2D_FILL );
      gle.SetColor(1,1,1,0.4);
      gle.Ellipse( v.x, v.y, 4,4,1,0,8);
    end;
end;
